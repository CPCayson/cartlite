      ...
      try {
        ...
        if (!stripeAccountId) {
          const account = await stripe.accounts.create({
            ...
            business_profile: {
              name: "cartRabbit",
              product_description: "Software Application",
        ...
        const accountLink = await stripe.accountLinks.create({
          ...
          refresh_url: process.env.STRIPE_REFRESH_URL || "https://cart-rabbit.com",
          return_url: process.env.STRIPE_RETURN_URL || "https://cart-rabbit.com",
...
app.post("/api/check-stripe-status", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const account =
    await stripe.accounts.retrieve(userData.stripeAccountId);
...
app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        logger("Error creating Stripe customer:", error);
    ...
    async (req, res) => {
      ...
      try {
        ...
        const booking = {
          ...
          email: req.user.email || "cpcayson@gmail.com",
        };
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (!booking) {
          return res
              .status(404)
              .json({success: false, error: "Booking data not found"});
        }
