...
const RideStatusTracker = ({ rideRequestId }) => {
  ...
  useEffect(() => {
    if (!rideRequestId) {
      console.warn('RideStatusTracker: No rideRequestId provided.');
      return;
    }
    ...
    const unsubscribe = onSnapshot(rideRequestRef, (docSnapshot) => {
      if (docSnapshot.exists()) {
        ...
        console.warn(`RideStatusTracker: No document found for ID: ${rideRequestId}`);
        setStatus('No Ride Found');
      }
    }, (error) => {
      console.error('RideStatusTracker: Error fetching ride request:', error);
  ...
  return (
    <Box p={4} bg="gray.200" rounded="md" mt={4}>
