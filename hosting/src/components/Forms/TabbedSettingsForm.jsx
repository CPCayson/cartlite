...
const TabbedSettingsForm = ({ userData, cartData, onSaveUserData, onSaveCartData }) => {
  const [profileAvatar, setProfileAvatar] = useState(userData?.profileAvatar || null);
  const [firstName, setFirstName] = useState(userData?.firstName || '');
  const [lastName, setLastName] = useState(userData?.lastName || '');
  const [email, setEmail] = useState(userData?.email || '');
  const [phoneNumber, setPhoneNumber] = useState(userData?.phoneNumber || '');
  const [capacity, setCapacity] = useState(cartData?.capacity || 3);
  const [description, setDescription] = useState(cartData?.description || '');
  const [features, setFeatures] = useState(cartData?.features || {
    airConditioner: false,
    electric: true,
    radio: false,
    storage: true,
  });
  const [cartImages, setCartImages] = useState(cartData?.images || [
    'https://via.placeholder.com/150',
    'https://via.placeholder.com/150',
    'https://via.placeholder.com/150',
  ]);
  const [stripeConnected, setStripeConnected] = useState(userData?.stripeAccountStatus === 'complete');
  ...
  const handleSaveProfile = () => {
    if (typeof onSaveUserData === 'function') {
      ...
      onSaveUserData(updatedUserData);
    } else {
      console.error('onSaveUserData is not a function');
    }
  };

  const handleSaveCartInfo = () => {
    if (typeof onSaveCartData === 'function') {
      ...
      onSaveCartData(updatedCartData);
    } else {
      console.error('onSaveCartData is not a function');
    }
  };
        ...
        <TabPanels>
          ...
          <TabPanel>
            <VStack spacing={4}>
              ...
              <Button colorScheme="teal" width="full" onClick={handleSaveProfile}>
                Save Profile
              </Button>
            </VStack>
          </TabPanel>
          ...
          <TabPanel>
            <VStack spacing={4}>
              <FormControl>
                ...
                <Input placeholder="Cart Name" defaultValue={cartData?.cartName || ''} />
              </FormControl>
              ...
              <Button colorScheme="teal" width="full" onClick={handleSaveCartInfo}>
                Save Golf Cart Info
              </Button>
            </VStack>
          </TabPanel>
          ...
          <TabPanel>
            <VStack spacing={4}>
              ...
              <FormControl>
                ...
                <Input placeholder="acct_..." readOnly value={userData?.stripeAccountId || ''} />
              </FormControl>
...
TabbedSettingsForm.propTypes = {
  ...
  onSaveCartData: PropTypes.func.isRequired,
};
